...
::: info 本章概要
从群论过渡到环论，我们引入带有两种运算（加法和乘法）的代数结构。环的理想扮演着群论中正规子群的角色——它们是构造商环的基础，也是 [域扩张理论](/chapters/05-field-extensions/) 的根基。
:::
...
4. **理想的角色**：在 [域扩张](/chapters/05-field-extensions/) 中，不可约多项式的理想 $(f(x))$ 给出域扩张 $K[x]/(f(x)) \cong K(\alpha)$。
:::
